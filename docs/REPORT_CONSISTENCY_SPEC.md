...
> **Upstream:** REPORT_ASSEMBLY_PLAN (section outline, writing rules), FIGURES_TABLES_CONTRACT (artifact definitions)
> **Downstream:** PRE_SUBMISSION_CHECKLIST (final delivery audit), audit_report.py (machine-checkable verification)
>
...
### Bullet List Restriction [AUTO]
- No list environment with >2 items in Results, Discussion, or Analysis sections
- 50% deduction risk (UL standard) or 50 points (SL/OL standard)
- Enforcement: gen_report_auditor.py check_bullet_lists
- Exception: Methods may use short lists (≤2 items) for algorithm/dataset descriptions
- Self-test: `grep -c '\\item' report.tex` should be minimal; `grep -n '\\begin{itemize}' report.tex` should return few hits
